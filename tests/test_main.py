...
def test_data_sources_endpoint():
    ...
    for source in data:
        ...
        assert (
            "available_columns" in source
        ), "Each data source should list available columns"
...
def test_technology_analysis_endpoint():
    ...
    if response.status_code == 200:
        ...
        assert isinstance(
            data["total_responses"], int
        ), "Total responses should be an integer"
        assert isinstance(
            data["unique_technologies"], int
        ), "Unique technologies should be an integer"
...
def test_technology_analysis_with_parameters():
    ...
    """
    response = client.get(
        "/api/analysis/technology-usage?source=stackoverflow_2023&column=LanguageHaveWorkedWith&top_n=5"
    )
    ...
    assert response.status_code in [
        ...
        404,
        400,
    ], f"Expected 200, 404, or 400, got {response.status_code}"
...
def test_invalid_column():
    ...
    """
    response = client.get(
        "/api/analysis/technology-usage?source=stackoverflow_2023&column=NonexistentColumn"
    )

    assert response.status_code in [
        400,
        404,
    ], "Should return 400 or 404 for invalid column"
...
def test_root_endpoint():
    ...
    ), f"Expected 200 status code for root endpoint, got {response.status_code}"
...
def test_api_parameter_validation():
    ...
    ), "Should return 422 for top_n parameter exceeding limit"
